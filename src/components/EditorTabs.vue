...
function closeEditorTab( closedFilePath ){
    editorHelper.closeEditorTab( closedFilePath );
}
...
<template>
    <div class="vdEditorPageTabs">
        ...
        <div class="vdEditorPageTabsWrapper">
            <template v-for="openFile in codeStore.openFiles" :key="openFile.path">
                <div :class="(openFile.path == codeStore.selectedObject) ? 'vdEditorPageTab active' : 'vdEditorPageTab'" >
                    <span class="vdEditorPageTabName" @click="getR2ObjectData(openFile.path)">{{ openFile.filename }}</span>
                    <span class="vdEditorPageTabClose vdEditorPageTabBtn" @click="closeEditorTab(openFile.path)">
